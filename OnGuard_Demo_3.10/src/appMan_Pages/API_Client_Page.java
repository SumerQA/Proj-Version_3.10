...
public class API_Client_Page {
	Common c = new Common();	 
	final WebDriver driver;
	public  API_Client_Page(WebDriver driver)
	{   
		this.driver = driver;
	}
	
	 public void Goto_ApiClient_page() throws IOException, InterruptedException {	
			...
			TC_page.API_Client_Tab();
			Thread.sleep(1500);
			HI_page.Render_Notifications();
		    System.out.println("API Client tab is clicked");
		    }
	 ...
	 public void API_Client_name(String text) throws IOException, InterruptedException {	
			events eve=new events(driver);
			eve.SendKey_event("api_client_name", text);       //Printer Name
	 ...
	 public void Token_Expiry(String date) throws IOException, InterruptedException {	
		 events eve=new events(driver);
